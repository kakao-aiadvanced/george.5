...
system = """You are an assistant for question-answering tasks.
    ...
    Use three sentences maximum and keep the answer concise. When creating an answer, I would like you to indicate the source of the answer. When creating your answer, please include the URL and title of the source you referred to."""
...
class GraphState(TypedDict):
    ...
    generation: str
    # web_search: str
    relevanceCheckStatus: str
    hallucinationCheckStatus: str
    documents: List[str]
...
def retrieve(state):
    ...
    print(question)
    print(documents)
    return {"documents": documents, "question": question, "relevanceCheckStatus": "NORMAL", "hallucinationCheckStatus": "NORMAL"}
...
def generate(state):
    ...
    print(state["hallucinationCheckStatus"])
...
def grade_documents(state):
    ...
    for d in documents:
        ...
        # Document not relevant
            ...
            if state["relevanceCheckStatus"] == "RETRY":
              return {"documents": filtered_docs, "question": question, "relevanceCheckStatus": "NOT_RELEVANT"}
            else:
              return {"documents": filtered_docs, "question": question, "relevanceCheckStatus": "RETRY"}

    return {"documents": filtered_docs, "question": question, "relevanceCheckStatus": "COMPLETE"}
...
def decide_to_generate(state):
    ...
    state["question"]
    relevanceCheckStatus = state["relevanceCheckStatus"]
    state["documents"]

    if relevanceCheckStatus == "RETRY":
        # All documents have been filtered check_relevance
        # We will re-generate a new query
        print(
            "---DECISION: ALL DOCUMENTS ARE NOT RELEVANT TO QUESTION, GO RETRY---"
        )
        return "retry"
    elif relevanceCheckStatus == "NOT_RELEVANT":
        print(
            "---DECISION: NOT RELEVANT TO QUESTION---"
        )
...
def grade_generation_v_documents_and_question(state):
    ...
    if grade == "yes":
        ...
        if state["hallucinationCheckStatus"] == "RETRY":
          pprint("---DECISION: Hallucination")
          print("failed: hallucination")
          return "hallucination"
        else:
          pprint("---DECISION: GENERATION IS NOT GROUNDED IN DOCUMENTS, RE-TRY---")
          state["hallucinationCheckStatus"] = "RETRY"
          # state = {"hallucinationCheckStatus": "RETRY", "question": question, "generation": generation, "documents": documents}
          return "retry"
